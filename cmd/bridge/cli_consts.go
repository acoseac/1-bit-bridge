...
const (
	...
	// wraps it; `bridge doctor` calls it directly, because it reports a
	// missing or unloadable config where loadCLIConfig errors), so a
	// command that parses --config and then calls config.Load directly
	// dies with `read config ""` on any host where the operator did not
	// pass the flag — the defect TestNoSubcommandTailBypassesLoadCLIConfig
	// sweeps for.
	//
