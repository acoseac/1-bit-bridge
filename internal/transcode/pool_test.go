...
func TestPoolEnqueueDeduplicates(t *testing.T) {
	...
	p.runner = func(ctx context.Context, _ JobSpec) (RunResult, error) {
		...
		return RunResult{SizeBytes: 0}, ctx.Err()
	}
...
func TestPoolFiresOnStateChangeAfterEnqueueAndCompletion(t *testing.T) {
	...
	p.runner = func(ctx context.Context, spec JobSpec) (RunResult, error) {
		if err := releaseGate(ctx, release); err != nil {
			return RunResult{SizeBytes: 0}, err
		}
		return RunResult{SizeBytes: 0}, errors.New("synthetic failure")
	}
...
func TestPoolPanicInRunnerReleasesDedup(t *testing.T) {
	...
	p.runner = func(ctx context.Context, spec JobSpec) (RunResult, error) {
		...
		case "Music/Album/survivor.flac":
			...
			// not the store path.
			return RunResult{SizeBytes: 0}, errors.New("expected: not a real source file")
		default:
			t.Errorf("unexpected runner spec: %q", spec.SourceLibraryRel)
			return RunResult{SizeBytes: 0}, nil
		}
...
func TestPoolFiresOnJobCompleteAfterUpsertVariant(t *testing.T) {
	...
	p.fsyncFn = noopFsync
	p.runner = func(ctx context.Context, spec JobSpec) (RunResult, error) {
		return RunResult{SizeBytes: 42}, nil
	}
...
func TestPoolDoesNotFireOnJobCompleteOnFailure(t *testing.T) {
	...
	p.runner = func(ctx context.Context, spec JobSpec) (RunResult, error) {
		...
		return RunResult{SizeBytes: 0}, soxErr
	}
