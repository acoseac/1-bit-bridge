...
func TestEnsureThumbWritesEvenWhenTheScaledFileIsLarger(t *testing.T) {
	...
	source := writeArtFixture(t, src, 400, 400, 5) // tiny bytes, 400 px
	...
	// The fixture's own precondition, asserted rather than assumed: if
	// the thumbnail came out SMALLER, the byte-length form this test
	// exists to reject would have written it too, and the test would
	// pass while pinning nothing. A different encoder — or a tweak to
	// the quality constant — could silently put it in that state.
	if len(out) <= len(source) {
		t.Fatalf("fixture no longer reproduces growth: source %d bytes, thumb %d — "+
			"this test cannot distinguish the dimensional check from the byte-length one",
			len(source), len(out))
	}
	...
	t.Logf("source %d bytes @400px -> thumb %d bytes @250px (growth is the point)",
		len(source), len(out))
}
