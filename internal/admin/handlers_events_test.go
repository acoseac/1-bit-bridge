...
// asserts the initial-snapshot publish lands all eight named events
...
func TestEventsStreamInitialSnapshot(t *testing.T) {
	...
	frames := readFrames(t, resp.Body, 8, 3*time.Second)
	if len(frames) != 8 {
		t.Fatalf("frames: got %d want 8 (got: %v)", len(frames), frames)
	}
	...
	for _, want := range []string{"stats", "pairing", "endpoints", "updates", "tailscale", "composition", "upscale", "analysis"} {
		if !seen[want] {
...
func TestEventsStreamDiffSuppression(t *testing.T) {
	...
	// Drain initial snapshot (8 named events incl. composition/upscale/analysis).
	initial := readFrames(t, resp.Body, 8, 3*time.Second)
	if len(initial) != 8 {
...
func TestEventsStreamWakesOnStateChange(t *testing.T) {
	...
	if got := readFrames(t, resp.Body, 8, 3*time.Second); len(got) != 8 {
