...
**Validate a config edit BEFORE restarting, never after** — `bridge doctor --config <path>` loads and validates it in a throwaway process, so a bad key is a non-zero exit (its `config-file` line FAILs and names the key) against a service that is still happily serving, and so is a `--config` path that is not there (a typo'd path used to grade defaults instead). That holds from #985 on. Before it, doctor dropped a config it could not load and graded defaults: "all clear", exit 0, on exactly the typo this step exists to catch. Run it as the service's user, or with read access to the file: a config doctor cannot READ is only a warn, since that is a fact about the run, not the file. From #1022 on, a config that did not load (unreadable, not there, or with a bad key) leaves the `port-api` / `port-admin` lines reading "not checked"; before, they graded 7788 / 7789 with no pid file and FAILed "another process owns this port" against the service's own listeners. Run as another user who cannot read the file, `config-dir` reads "not checked" too from #1023 on; before, its write probe FAILed as that user, so such a run exited 1 for a reason that was not about the config. A user who CAN read the file but not write its directory still gets config-dir's FAIL ("not writable"), and that line is about the run as well: the config's verdict is the `config-file` line. From #1029 on, an edit that moves `listenAddress` or `adminAddress` to a port another process holds is a FAIL too, while the old bridge still runs, wherever doctor can read that bridge's descriptors: on Windows always, and on Linux when it runs as the service's user. Before, that line read ok (on Linux, where the holder ran as the user running doctor) or warned, and doctor exited 0 either way. **On a capability-bound bridge (the NUC)** the service's user cannot read that bridge's descriptors (it runs with dumpable=0). From #1030 on, doctor FAILs such an edit anyway when every process holding the new port is one the service's user CAN read (its own processes, which are dumpable): those holders hold every listener on the port, so the bridge holds none. Before, a holder running as the service's user read ok. From #1032 on it FAILs too when the holder runs as ANOTHER user, root's daemon included (the likelier shape on the NUC), which the service's user cannot read: the socket tables say which uid created each listener, the bridge creates its own as its own uid, and a listener another uid created is not the bridge's. Before, that shape warned, exit 0. From #1033 on it FAILs too when the holder runs as the service's OWN user but the service's user cannot read it either (another capability-bound binary of that user, or one of its processes in another group), provided it runs in another cgroup than the bridge: another systemd unit, a container, or another login session. The kernel reports the cgroup each listener was created in (what `ss -ltn --cgroup` shows), and the bridge creates its own in its unit's. Before, that shape read ok, exit 0. **Two shapes doctor still cannot tell from the bridge**: such a holder INSIDE the bridge's own cgroup (a second process the bridge's unit starts, or one started from the same login session as a hand-started bridge) reads ok, and on a host whose `/proc` is mounted with `hidepid=1` or `2` a holder of another user still warns, since hidepid hides the bridge's status too. A bridge that root has moved into another cgroup by hand after it started (writing its pid into another cgroup's `cgroup.procs`) FAILs its own ports from #1033 on, the hint naming both cgroups; a restart clears it. So when an edit MOVES a port, read that port's line rather than the verdict: the running bridge is still on the old port, so the new one must read `free (:<port>)`, and anything else there is another process, whatever doctor attributes it to. Check such a port by hand (`ss -ltnp --cgroup 'sport = :<port>'` as root) before restarting. Restarting first turns the same typo into a crash-loop you then have to diagnose from the journal. For the same reason, **deploy the binary first and edit the config second** when a release adds keys: the running (old) binary never sees a key it does not know, and the deploy stays independently verifiable before any behaviour changes.
