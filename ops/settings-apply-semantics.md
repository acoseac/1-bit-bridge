...
| `atlasLyricsEnabled` | **A** | `live` (+reason when Atlas/harvest are off) | The sweep's sink is wired unconditionally and the flag is read per tick, through `config.AtlasConfig.LyricsTierActive` — the same predicate `/api/jobs` renders the card from, so the console and the sweeper cannot disagree. They did: the sweeper took the flag at boot, inside the `if` that decided whether to wire the sink, while the console read it live. Invisible while a restart was the only way to change the value, and this row is what would have made it bite. Rides the harvest credential, so a bridge without one saves the flag and stays inert — reported with a reason, never silently. |
