...
- **`bridge doctor` never tries `./bridge.yaml`, so inside the container it
  needs `--config /data/bridge.yaml`.** It is the one subcommand that does not
  go through `loadCLIConfig`: `buildDoctorDeps` loads an explicit `--config` or
  else the platform path (`~/.config/1-bit-bridge/`, absent in the image). A
  plain `docker exec … bridge doctor` therefore grades an install with NO
  config — no data dir, so no `server.pid` to attribute the bridge's own
  listeners, and both port checks FAIL "another process owns this port"; no env
  overrides either, so `audio-toolchain` reads "not enabled" beside
  `BRIDGE_UPSCALE_ENABLED=true`. With the flag both ports read `bound by our
  own bridge (pid 1)`. The Dockerfile and docs/docker.md blamed the FAIL on
