...
**v0.1.5** — wire protocol stays at [`PROTOCOL.md`](PROTOCOL.md) v1 (additive only since v0.1.4: optional `X-Device-Token` durable device identity; `GET /v1/playlists` + `GET/PUT/DELETE /v1/playlists/{id}`, `POST /v1/history/batch`, and `GET /v1/renderers` endpoints; `playlistBackup` / `playbackHistory` / `dlnaServer` / `rendererDiscovery` capability flags on `GET /v1/health`). Feature additions: optional **DLNA / UPnP MediaServer** (LAN-only, off by default) for Chord 2Go and generic UPnP renderers, **per-device playlist backup** that survives reinstalls, **opt-in listening history** surfaced only in the admin console, and an admin **Data** page to view / export both. `sudo bridge update` now tolerates a FUSE-mounted library root that's unreadable by root. macOS binaries are Developer-ID-signed and Apple-notarized; Windows binaries are unsigned for now (Authenticode signing via SignPath Foundation pending). See the [releases page](https://github.com/acoseac/1-bit-bridge/releases) for per-release notes.
