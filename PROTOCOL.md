...
`demoMode` (additive, v0.1.9) advertises the read-only public demo posture (`demo.enabled` in bridge.yaml — the `bridge.1-bit.app` fixture behind the iOS app's "Add demo bridge" action). On such a bridge the user-data stores are deliberately unwired, so `playlistBackup`, `playlistsCrossDevice`, `favorites`, `playbackHistory` and `playbackHistoryRead` are honestly absent from this array and their mutation endpoints return the same typed `404`s a feature-off bridge returns. Clients should treat `demoMode` as "label the source as a demo and hard-lock the sync toggles off" — presenting sync controls the server would refuse is the failure this flag exists to prevent. Everything read-side (manifest, streaming, artwork, waveforms, analysis scalars, smart playlists) behaves normally — including SERVING upscaled / CarPlay-optimized variants when the operator enabled upscaling. The four upscale MUTATION endpoints (`POST /v1/upscale`, `POST /v1/upscale/batch`, `DELETE /v1/upscale/batches/{id}`, `DELETE /v1/upscale/variants`) return `403` `demo_read_only` on a demo bridge regardless of bearer: its token is effectively public, so submitting server work must be impossible even though the feature flags advertise the (read-side) capability. `POST /v1/atlas-ingest` returns the same `403` `demo_read_only` there (v0.1.9): the ingest trusts the authenticated client's PAYLOAD, and a public bearer would let anyone poison the bios served to every demo user. `POST /v1/atlas-harvest/credential` and the `/v1/atlas-meta/*` reads stay open — a demo bridge gets its rich-tier metadata exclusively via its own harvest pull from Atlas (authentic, server-to-server), bootstrapped by an attested client handing it a capability token. `ProtocolVersion` stays `1` — the flag is additive and the absences are the existing feature-off shapes.
...
- `bpm` is the **existing** tempo field. It stays tag-first: a curated TBPM / BPM / `tmpo` tag always wins; the estimated tempo is spliced in **only when the source has no BPM tag**. A bare `bpm` value is therefore tag-curated *or* estimated, indistinguishable on its own (same contract as `replayGainTrackDB`) — which is what `bpmEstimated` below exists to resolve.
- `bpmEstimated` (additive, since v1.10) is `true` **only** when the served `bpm` is the analyzer's estimate — set at the same tag-absent-only splice, never on a curated tag. It is the client's licence to label the tempo "estimated": **only a positively-marked value may carry the label.** Absence makes no claim — a curated tag, a pre-`bpmEstimated` bridge, and a track with no tempo at all are indistinguishable without it, and a client must render all three with **no** label rather than guess (mislabelling a user's curated tag as an estimate is the failure this field exists to prevent). `omitempty`; `ProtocolVersion` stays `1`.
...
**An absent `bandwidthHz` means NO ANSWER — never "no content up there".** Beyond the ordinary cases (analysis hasn't reached the track; it was too short to average), there is one routine and important reason for absence: the bridge decodes at **48 kHz**, so it can only see to **24 kHz** — and its decode resampler's passband ends at **~22.8 kHz** (95% of Nyquist), so anything a file carries above that is reshaped by the bridge's own filter before it is measured. When a file's content reaches that region, the bridge reports nothing rather than a number: a measured "stop" inside the filter's transition band is the filter's crossing, not the file's. It must: the whole region reads as 48 kHz's Nyquist — any reported value in 22.8–24 kHz would let a client conclude "consistent with a 48 kHz source" and accuse a genuinely 96 kHz-native master on the strength of the *bridge's own decode filter*. A track can therefore carry a spectrum and no `bandwidthHz`, and that pairing is complete, not partial.
...
An id can also **leave** `deletedIds` without any client having written to it. `DELETE` only sets a flag — the row and its items are never removed — and the bridge's operator console can lift that flag again (Playlists → *Recently deleted*). A restored playlist returns to `playlists` with its stored `lastModifiedAt` **unchanged**: the operator undid a delete, they did not author a new version, so LWW still resolves exactly as it would have had the delete never happened. Clients need no new code for this — it is indistinguishable from a revive by `PUT`, and the id simply imports again on the next sweep. Two consequences worth knowing: a client holding its own **queued/pending delete** for that id will retry it and re-tombstone the playlist (drop the pending record once the bridge has answered, rather than replaying it against a later listing), and a custom cover uploaded for the playlist is **not** restored — `DELETE` unlinks it, so a restored playlist falls back to the auto-mosaic until one is uploaded again.
